...
## Declaring environment variables [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#declaring-environment-variables "Direct link to Declaring environment variables")
...
If using the UI, you can also [export locally-scoped variables to a `.env` file](https://docs.dagster.io/dagster-plus/deployment/management/environment-variables/dagster-ui#export), which you can then use to develop locally.

Refer to the [Dagster+ environment variables guide](https://docs.dagster.io/dagster-plus/deployment/management/environment-variables/) for more info.
...
- [Amazon Web Services EC2 / ECS](https://docs.dagster.io/guides/deploy/deployment-options/aws)
- [GCP](https://docs.dagster.io/guides/deploy/deployment-options/gcp)
- [Docker](https://docs.dagster.io/guides/deploy/deployment-options/docker)
- [Kubernetes](https://docs.dagster.io/guides/deploy/deployment-options/kubernetes/deploying-to-kubernetes)

## Accessing environment variables [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#accessing-environment-variables "Direct link to Accessing environment variables")
...
- [In Python code](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#in-python-code), which isn't specific to Dagster
- [From Dagster configuration](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#from-dagster-configuration), which incorporates environment variables into the Dagster config system

### In Python code [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#in-python-code "Direct link to In Python code")
...
This approach also works for accessing [built-in environment variables for Dagster+](https://docs.dagster.io/dagster-plus/deployment/management/environment-variables/built-in):
...
For a real-world example, see the [Dagster+ branch deployments example](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#dagster-branch-deployments).
...
### From Dagster configuration [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#from-dagster-configuration "Direct link to From Dagster configuration")

[Configurable Dagster objects](https://docs.dagster.io/guides/operate/configuration/run-configuration) \- such as ops, assets, resources, I/O managers, and so on - can accept configuration from environment variables. Dagster provides a native way to specify environment variables in your configuration. These environment variables are retrieved at launch time, rather than on initialization as with `os.getenv`. Refer to the [next section](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#using-envvar-vs-osgetenv) for more info.
...
Refer to the [Handling secrets section](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#handling-secrets) and [Per-environment configuration example](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#per-environment-configuration-example) for examples.

### Using EnvVar vs os.getenv [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#using-envvar-vs-osgetenv "Direct link to Using EnvVar vs os.getenv")
...
- **When `os.getenv` is used**, the variable's value is retrieved when Dagster loads the [code location](https://docs.dagster.io/guides/deploy/code-locations/) and **will** be visible in the UI.
...
## Handling secrets [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#handling-secrets "Direct link to Handling secrets")

Using environment variables to provide secrets ensures sensitive info won't be visible in your code or the launchpad in the UI. In Dagster, best practice for handling secrets uses [configuration](https://docs.dagster.io/guides/operate/configuration/run-configuration) and [resources](https://docs.dagster.io/guides/build/external-resources/).
...
- By subclassing [`ConfigurableResource`](https://docs.dagster.io/api/dagster/resources#dagster.ConfigurableResource) and specifying the `access_token` field, we're telling Dagster that we want to be able to configure the resource with an `access_token` parameter
...
- We're adding that resource to our [`Definitions`](https://docs.dagster.io/api/dagster/definitions#dagster.Definitions) object so it's available for our assets.

## Parameterizing pipeline behavior [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#parameterizing-pipeline-behavior "Direct link to Parameterizing pipeline behavior")

Using environment variables, you define how your code should execute at runtime.

- [Per-environment configuration example](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#per-environment-configuration-example)

### Per-environment configuration example [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#per-environment-configuration-example "Direct link to Per-environment configuration example")
...
This example is adapted from the [Transitioning data pipelines from development to production guide](https://docs.dagster.io/guides/deploy/dev-to-prod):
...
- We've created a dictionary of resource definitions, `resources`, named after our `local` and `production` environments. In this example, we're using a [Pandas Snowflake I/O manager](https://docs.dagster.io/api/libraries/dagster-snowflake-pandas).
...
### Dagster+ branch deployments [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#dagster-branch-deployments "Direct link to Dagster+ branch deployments")

note

This section is only applicable to Dagster+.

This example demonstrates how to determine the current deployment type at runtime - [branch deployment](https://docs.dagster.io/dagster-plus/features/ci-cd/branch-deployments/) or [full deployment](https://docs.dagster.io/dagster-plus/deployment/management/deployments/) \- without using resources or configuration.
...
This function checks the value of `DAGSTER_CLOUD_IS_BRANCH_DEPLOYMENT` and, if equal to `1`, returns a variable with the value of `branch`. This indicates that the current deployment is a branch deployment. Otherwise, the deployment is a full deployment and is\_branch\_depl will be returned with a value of prod.

Using this info, we can write code that executes differently when in a branch deployment or a full deployment.

## Troubleshooting [​](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets\#troubleshooting "Direct link to Troubleshooting")

| Error | Description | Solution |
| --- | --- | --- |
| **You have attempted to fetch the environment variable "\[variable\]" which is not set. In order for this execution to succeed it must be set in this environment.** | Surfacing when a run is launched in the UI, this error means that an environment variable set using [`StringSource`](https://docs.dagster.io/api/dagster/config#dagster.StringSource) could not be found in the executing environment. | Verify that the environment variable is named correctly and accessible in the environment.<br>- **If developing locally and using a `.env` file**, try reloading the workspace in the UI. The workspace must be reloaded any time this file is modified for the UI to be aware of the changes.<br>- **If using Dagster+**:<br>  - Verify that the environment variable is [scoped to the environment and code location](https://docs.dagster.io/dagster-plus/deployment/management/environment-variables/dagster-ui#scope) if using the built-in secrets manager<br>  - Verify that the environment variable was correctly configured and added to your [agent's configuration](https://docs.dagster.io/dagster-plus/deployment/management/environment-variables/agent-config) |
...
- [Declaring environment variables](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#declaring-environment-variables)
- [Accessing environment variables](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#accessing-environment-variables)
  - [In Python code](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#in-python-code)
  - [From Dagster configuration](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#from-dagster-configuration)
  - [Using EnvVar vs os.getenv](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#using-envvar-vs-osgetenv)
- [Handling secrets](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#handling-secrets)
- [Parameterizing pipeline behavior](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#parameterizing-pipeline-behavior)
  - [Per-environment configuration example](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#per-environment-configuration-example)
  - [Dagster+ branch deployments](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#dagster-branch-deployments)
- [Troubleshooting](https://docs.dagster.io/guides/deploy/using-environment-variables-and-secrets#troubleshooting)

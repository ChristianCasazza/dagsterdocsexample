...
Dagster can detect hanging runs and restart crashed [run workers](https://docs.dagster.io/guides/deploy/oss-deployment-architecture#job-execution-flow). Using run monitoring requires:
...
In Dagster+, run monitoring is always enabled and can be configured in [deployment settings](https://docs.dagster.io/dagster-plus/deployment/management/deployments/deployment-settings-reference)

## Run start timeouts [​](https://docs.dagster.io/guides/deploy/execution/run-monitoring\#run-start-timeouts "Direct link to Run start timeouts")
...
## Run cancelation timeouts [​](https://docs.dagster.io/guides/deploy/execution/run-monitoring\#run-cancelation-timeouts "Direct link to Run cancelation timeouts")
...
or in Dagster+, add the following to your [deployment settings](https://docs.dagster.io/dagster-plus/deployment/management/deployments/deployment-settings-reference):
...
## Detecting run worker crashes [​](https://docs.dagster.io/guides/deploy/execution/run-monitoring\#detecting-run-worker-crashes "Direct link to Detecting run worker crashes")

note

Detecting run worker crashes only works when using a run launcher other than the [`DefaultRunLauncher`](https://docs.dagster.io/api/dagster/internals#dagster._core.launcher.DefaultRunLauncher).
...
## Resuming runs after run worker crashes [​](https://docs.dagster.io/guides/deploy/execution/run-monitoring\#resuming-runs-after-run-worker-crashes "Direct link to Resuming runs after run worker crashes")

This feature is currently only supported when using:

- [`K8sRunLauncher`](https://docs.dagster.io/api/libraries/dagster-k8s#dagster_k8s.K8sRunLauncher) with the [`k8s_job_executor`](https://docs.dagster.io/api/libraries/dagster-k8s#dagster_k8s.k8s_job_executor)
- [`DockerRunLauncher`](https://docs.dagster.io/api/libraries/dagster-docker#dagster_docker.DockerRunLauncher) with the [`docker_executor`](https://docs.dagster.io/api/libraries/dagster-docker#dagster_docker.docker_executor)
...
- [Run start timeouts](https://docs.dagster.io/guides/deploy/execution/run-monitoring#run-start-timeouts)
- [Run cancelation timeouts](https://docs.dagster.io/guides/deploy/execution/run-monitoring#run-cancelation-timeouts)
- [General run timeouts](https://docs.dagster.io/guides/deploy/execution/run-monitoring#general-run-timeouts)
- [Detecting run worker crashes](https://docs.dagster.io/guides/deploy/execution/run-monitoring#detecting-run-worker-crashes)
- [Resuming runs after run worker crashes](https://docs.dagster.io/guides/deploy/execution/run-monitoring#resuming-runs-after-run-worker-crashes)
